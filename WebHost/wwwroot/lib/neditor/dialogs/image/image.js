...
(function () {
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'upload':
                    ...
                    var count = uploadImage.getQueueInfo().count;
                    if (count) {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            var compress = editor.getOpt('imageCompressEnable') ? {
            ...
            function updateStatus() {
                var text = '', stats;
                var info = _this.getQueueInfo();
                var fileCount = info.count;
            ...
            uploader.on('fileQueued', function (file) {
                ...
                var imageUploadService = editor.getOpt("imageUploadService");
                if (imageUploadService) {
                    imageUploadService = imageUploadService(_this, editor);
                    if (imageUploadService.setUploadData) imageUploadService.setUploadData(file);
                }
                var fileCount = _this.getQueueInfo().count;
                if (fileCount === 1) {
                    $placeHolder.addClass('element-invisible');
                    $statusBar.show();
                }
                ...
                switch (type) {
                    case 'uploadBeforeSend':
                        if(this.options.compress){
                            var file = files.file;
                            var fileSizeLimit = editor.getOpt('imageMaxSize');
                            if(file.size > fileSizeLimit){
                                this.skipFile(file);
                                stats = this.request('get-stats');
                                stats.numOfSuccess--;
                                stats.numOfInvalid++; 
                                $("#"+file.id + " .error").text(lang.errorExceedSize + (fileSizeLimit / 1024) + "KB").show();
                                return false;
                            }
                        }
                        break;
                    ...
                    case 'startUpload':
                        ...
                        if (imageUploadService) {
                            imageUploadService = imageUploadService(_this, editor);
                            if (imageUploadService.setUploaderOptions) {
            ...
            uploader.on('uploadBeforeSend', function (object, data, headers) {
                ...
                if (imageUploadService) {
                    imageUploadService = imageUploadService(_this, editor);
                    if (imageUploadService.setFormData) imageUploadService.setFormData(object, data, headers);
                }
            ...
            uploader.on('uploadSuccess', function (file, res) {
                ...
                try {
                    ...
                    if (imageUploadService) {
                        imageUploadService = imageUploadService(_this, editor);
                        if (imageUploadService.getResponseSuccess) {
                            if (imageUploadService.getResponseSuccess(res)) {
        ...
        getQueueInfo: function () {
            var file, i, status, readyFile = 0, size = 0, files = this.uploader.getFiles();
            ...
            return { count: readyFile, size: size };
        },
        destroy: function () {
            this.$wrap.remove();
        },
        getInsertList: function () {
            ...
            var imageUploadService = editor.getOpt("imageUploadService");
            if (imageUploadService) {
                imageUploadService = imageUploadService(this, editor);
                imageSrcField = imageUploadService.imageSrcField;
            }
            imageSrcField = imageSrcField || 'url';
