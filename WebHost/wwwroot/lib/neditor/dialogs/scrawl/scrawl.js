...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        var bolExcute = false;
        var scrawlUploadService = editor.getOpt("scrawlUploadService");
        if (scrawlUploadService) {
            scrawlUploadService = scrawlUploadService(scrawlObj, editor);
            if (scrawlUploadService.uploadScraw) {
                var file = scrawlObj.dataURLtoFile(base64, 'scrawl-image.png');
                scrawlUploadService.uploadScraw(file, base64, function (data) {
                    if (!scrawlObj.isCancelScrawl) {
                        if (data.responseSuccess) {
                            var imgObj = {},
                                url = editor.options.scrawlUrlPrefix + data[data.scrawlSrcField || "url"];
                            imgObj.src = url;
                            imgObj._src = url;
