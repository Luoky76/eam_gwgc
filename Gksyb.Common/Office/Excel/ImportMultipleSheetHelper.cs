...
namespace Gksyb.Common.Office.Excel
{
    ...
    public class ImportMultipleSheetHelper : IDisposable
    {
        ...
        public Task<ImportResult<object>> Import(string sheetName, int sheetIndex, Type importDataType, bool isSaveLabelingError = true)
        {
            ...
            try
            {
                _excelPackage ??= new ExcelPackage(_excelStream);
                PreImport?.Invoke(ExcelImporterSettings, _excelPackage);
                {
                    #region 检查模板
        ...
        protected virtual bool ParseImporterHeader(Type sheetType)
        {
            ...
            foreach (var propertyInfo in objProperties)
            {
                ...
                //为bool类型生成默认映射
                var realType = propertyInfo.PropertyType.GetUnNullableType();
                ...
                if (isNullable) type = type.GetUnNullableType();
                //为枚举类型生成默认映射
                if (type.IsEnum)
                {
            ...
            foreach (var sheetProperty in _sheetPropertyList)
            {
                ...
                for (var i = 0; i < ImporterHeaderInfos.Count; i++)
                {
                    ...
                    if (ImporterHeaderInfos[i].IsRequired)
                        worksheet.Cells[importerAttribute.HeaderRowIndex, i + 1].Style.Font.Color.SetColor(SKColors.Red);
                ...
                worksheet.Cells[worksheet.Dimension.Address].Style.Fill.BackgroundColor.SetColor(SKColors.DarkSeaGreen);
            }
        }
                ...
                if (isNullNumber < worksheet.Dimension.End.Column)
                {
                        ...
                        ImporterHeaderInfos.Any(p1 => p1.PropertyName == p.Name && p1.IsExist)))
                    {
                        ...
                        try
                        {
                            ...
                            if (ImportHelper<object>.TryCastTo(cell, propertyInfo.PropertyType, out object myresult))
                            {
                                ...
                                propertyInfo.SetValue(dataItem, myresult);
                            }
                            else
                            {
