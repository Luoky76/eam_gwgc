...
namespace Gksyb.Common.Office.Excel
{
    ...
    public class ImportHelper<T> : IDisposable where T : class, new()
    {
        ...
        private Dictionary<string, dynamic> dicMergePreValues = new();
        public Action<ExcelImporterAttribute, ExcelPackage> PreImport = null;
        /// <summary>
        /// </summary>
        ...
        public Task<ImportResult<T>> Import(string filePath = null, Func<ImportResult<T>, ImportResult<T>> importResultCallback = null)
        {
            ...
            try
            {
                ...
                using (Stream)
                {
                    ...
                    PreImport?.Invoke(ExcelImporterSettings, excelPackage);
                    #region 检查模板
                    ...
                    #endregion 执行结果回调 zhenhua.shen 2022.1.7
        ...
        {
            if (ExcelImporterSettings.IsDisableAllFilter || filterType == null) return null;
            ...
            if (ExcelImporterSettings.IsLabelingError && ImportResult.HasError)
            {
                ...
                foreach (var item in ImportResult.RowErrors)
                {
                    ...
                    foreach (var field in item.FieldErrors)
                    {
                        ...
                        cell.Style.Font.Color.SetColor(SKColors.Red);
        ...
        internal virtual void LabelingBussinessError(ExcelPackage excelPackage, List<DataRowErrorInfo> bussinessErrorDataList, out string filePath)
        {
            ...
            LabelingBussinessError(excelPackage, bussinessErrorDataList);
        ...
        /// <param name="excelPackage"></param>
        /// <param name="bussinessErrorDataList"></param>
        private void LabelingBussinessError(ExcelPackage excelPackage, List<DataRowErrorInfo> bussinessErrorDataList)
        {
        ...
        {
            ParseTemplate(excelPackage, ImportResult);
        }
        ...
        protected virtual void StructureExcel(ExcelPackage excelPackage)
        {
            ...
            for (var i = 0; i < ImporterHeaderInfos.Count; i++)
            {
                ...
                if (ImporterHeaderInfos[i].IsRequired)
                    worksheet.Cells[ExcelImporterSettings.HeaderRowIndex, i + 1].Style.Font.Color.SetColor(SKColors.Red);
            ...
            //绿色太丑了
            worksheet.Cells[worksheet.Dimension.Address].Style.Fill.BackgroundColor.SetColor(SKColors.White);
        }
        ...
        private static void SetFormat(ExcelWorksheet worksheet, string format)
        {
        ...
        /// </remarks>
        private static void SetInterValidation(ExcelWorksheet worksheet, PropertyInfo propertyInfo, string address, string showInputMessage)
        {
        ...
        /// <returns></returns>
        private static ExcelPicture GetImage(ExcelWorksheet worksheet, int position)
        {
        ...
        /// <returns></returns>
        private static ExcelPicture GetImage(ExcelWorksheet worksheet, int row, int column)
        {
        ...
        {
            ImportResult.Data = ParseDataEnumer(excelPackage).ToList();
        }
        ...
        protected virtual IEnumerable<T> ParseDataEnumer(ExcelPackage excelPackage)
        {
            ...
            if (ExcelImporterSettings.MaxCount != 0
                ...
                ) throw new ArgumentException($"最大允许导入条数不能超过{ExcelImporterSettings.MaxCount}！");
                    ...
                    try
                    {
                        ...
                        else
                        {
                            if (col.ImportImageFieldAttribute != null)
                            {
                                ...
                                var path = Path.Combine(col.ImportImageFieldAttribute.ImageDirectory, Guid.NewGuid() + "." + ExcelImage.GetFileExtension(excelPicture.ImageFormat));
                                ...
                                switch (col.ImportImageFieldAttribute.ImportImageTo)
                                {
                                    ...
                                    case ImportImageTo.Base64:
                                        value = excelPicture.Image.ToBase64String();
                                        break;
                        ...
                        if (TryCastTo(cell, propertyInfo.PropertyType, out object myresult))
                        {
                            if (propertyInfo.PropertyType == typeof(string))
                            {
                                if (col.Header.FixAllSpace)
                                    myresult = (myresult as string)?.Replace(" ", string.Empty);
                                else if (col.Header.AutoTrim)
                                    myresult = (myresult as string)?.Trim();
                            }
                            SetValue(cell, dataItem, propertyInfo, myresult);
                        }
                        else
                        {
                            AddRowDataError(rowIndex, col, $"值 {cell.Text} 无效，正确的格式为{propertyInfo.PropertyType.Name}！");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        internal static bool TryCastTo(ExcelRange cell, Type type, out object result)
        {
            var value = cell.Value;
            var realType = type.GetUnNullableType();
            try
            {
                var isValueNull = value == null;
                var isNullable = type.IsNullableType() || realType == typeof(string);
                if (isValueNull && isNullable)
                {
                    result = null;
                    return true;
                }
                var valueType = value?.GetType();
                if (valueType == type || valueType == realType)
                {
                    result = value;
                    return true;
                }
                ...
                {
                    if (TimeSpan.TryParse(text, out var dt))
                    {
                        result = dt;
                        return true;
                    }
                ...
                else if (realType == typeof(DateTimeOffset))
                {
                    if (value is double v)
                    {
                        result = new DateTimeOffset(DateTime.FromOADate(v));
                        return true;
                    }
                ...
                result = Convert.ChangeType(value, realType);
                return true;
            }
            catch (Exception)
            {
                ...
                try
                {
                    result = Convert.ChangeType(cell.Text, realType);
                    return true;
                }
                catch (Exception)
                {
