    ...
    public static class DbBulkInsertExtension
    {
        ...
        public static async Task BulkInsertAsync<T>(OracleContext source, List<T> entities, int? batchSize = null, int? bulkCopyTimeout = null)
        {
            ...
            try
            {
                ...
                var version = (persistedConnection.ServerVersion ?? "").Split(".").FirstOrDefault().CastTo<int?>(11);
                if (version < 11)
                {
                    await source.InsertRangeAsync(entities);
                    return;
        ...
        {
            DataTable dt = null;
            var connection = source.Session.CurrentConnection as SqlConnection;
            var isOpen = false;
            try
            {
                if (connection.State == ConnectionState.Closed)
                ...
                dt = ToDataTable(entities);
                var bulkCopy = new SqlBulkCopy(connection, SqlBulkCopyOptions.Default, source.Session.CurrentTransaction as SqlTransaction)
                {
                    DestinationTableName = dt.TableName
                };
                if (batchSize.HasValue) bulkCopy.BatchSize = batchSize.Value;
        ...
        private static DataTable ToDataTable<T>(List<T> entities)
        {
            ...
            {
                dt.Columns.Add(c.Column.Name, c.Column.Type.GetUnNullableType());
            });
            entities.ForEach(c =>
            {
